...
async function runApp(config){
 ...
 app.whenReady().then(async()=>{
  try{
   ...
   await reveal('[aria-labelledby="usage-heading"]');await shot('actual-settings-usage',['#usage-heading','.usage-overview','.usage-footer']);
   check((await visible('.usage-overview')).visible&&(await visible('.usage-footer')).visible&&(await run(`document.querySelector('.usage-balance')===null`)),'local usage and billing link are visible without unsupported balance placeholder');
   await reveal('[aria-labelledby="recording-heading"]');await shot('actual-settings-recording',['#recording-heading','[aria-label="自动区分说话人"]','[aria-label="共用麦克风"]']);
   check(await run(`document.querySelectorAll('[aria-labelledby="recording-heading"] .set-card').length===1&&document.querySelectorAll('[aria-labelledby="recording-heading"] .set-card>.set-row').length===3`),'recording settings share one card with three divided rows');
   check((await snap()).autoTitle===false&&await run(`document.querySelector('[aria-label="自动生成录音标题"]').getAttribute('aria-checked')==='false'`),'automatic recording titles default off for legacy preferences through real snapshot and UI');
   await reveal('[aria-label="自动生成录音标题"]');
   await run(`document.querySelector('[aria-label="自动生成录音标题"]').click()`);await until(`window.earshot.snapshot().then(s=>s.autoTitle===true)`);
   check(JSON.parse(fs.readFileSync(join(config.support,'prefs.json'),'utf8')).autoTitle===true,'automatic-title switch persists enabled preference through actual preload and IPC');
   await new Promise(resolve=>{window.webContents.once('did-finish-load',resolve);window.webContents.reload();});await until(`Boolean(window.earshot&&document.querySelector('.slist'))`);
   check((await snap()).autoTitle===true,'automatic-title preference survives renderer reload');
   await run(`document.querySelector('.sl-foot button').click()`);await until(`Boolean(document.querySelector('[aria-label="自动生成录音标题"]'))`);
   await run(`document.querySelector('[aria-label="自动生成录音标题"]').click()`);await until(`window.earshot.snapshot().then(s=>s.autoTitle===false)`);
   check(JSON.parse(fs.readFileSync(join(config.support,'prefs.json'),'utf8')).autoTitle===false,'automatic-title switch persists disabled preference through real IPC');
   check([config.a,config.b,config.d].every(id=>!JSON.parse(fs.readFileSync(join(config.support,'sessions',id,'session.json'),'utf8')).autoTitle),'enabling and disabling leaves all historical and imported fixture sessions un-enrolled');
   ...
   // Real session IPC and persisted directories, using only the three synthetic fixtures.
   await run(`(()=>{const e=document.querySelector('input[type="search"]');Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(e,'');e.dispatchEvent(new Event('input',{bubbles:true}));})()`);
   await run(`document.querySelector('[data-session-id="'+${JSON.stringify(config.a)}+'"] > .sl-row').click()`);
   await until(`window.earshot.snapshot().then(s=>s.selectedId===${JSON.stringify(config.a)})`);
   await run(`document.querySelector('[data-session-id="'+${JSON.stringify(config.a)}+'"] > .sl-row').dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true}))`);
   await until(`Boolean(document.querySelector('.session-row input'))`);
   await input('.session-row input','实际 IPC 改名验证');await run(`document.querySelector('.session-row form').requestSubmit()`);
   await until(`window.earshot.snapshot().then(s=>s.selected.title==='实际 IPC 改名验证')`);
   check(JSON.parse(fs.readFileSync(join(config.support,'sessions',config.a,'session.json'),'utf8')).title==='实际 IPC 改名验证','double-click rename persists through real IPC');
   await run(`document.querySelector('[data-session-id="'+${JSON.stringify(config.d)}+'"] > .sl-row').dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,metaKey:true}))`);
   await until(`document.querySelectorAll('.session-row.selected').length===2`);
   await run(`document.querySelector('.session-selection-bar button').click()`);
   await until(`window.earshot.snapshot().then(s=>s.sessions.length===1&&s.deletions.length===2)`);
   check([config.a,config.d].every(id=>!fs.existsSync(join(config.support,'sessions',id))&&fs.existsSync(join(config.support,'pending-deletions',id))),'mixed batch deletes only selected recording and dictation into recoverable directories');
   check((await snap()).sessions[0].id===config.b,'unselected recording survives batch deletion');
   await until(`Boolean(document.querySelector('.deletion-notice button:not(:disabled)'))&&document.querySelector('.deletion-notice')?.textContent.includes('2 个会话')`);
   await run(`document.querySelector('.deletion-notice button').click()`);
   await until(`window.earshot.snapshot().then(s=>s.sessions.length===3&&s.deletions.length===0)`);
   check([config.a,config.d].every(id=>fs.existsSync(join(config.support,'sessions',id))&&!fs.existsSync(join(config.support,'pending-deletions',id))),'single undo action restores both real fixture directories');
