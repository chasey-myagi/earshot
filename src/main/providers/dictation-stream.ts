...
export function startDictationStream(opts: { apiKey: string; model: ModelPrefs['asr']; signal: AbortSignal; connect?: SocketConnect }) {
  const vocabulary = hotwordParameters(opts.model, opts.apiKey);
  ...
  let transmitted = 0;
  function meter(raw?: unknown, complete = false) {
    const usage = parseUsage(raw);
    if (!transmitted && usage.audioSeconds === undefined) return;
    recordUsage({ id: taskId, at: usageAt, model: opts.model, kind: 'dictation-asr', audioSeconds: usage.audioSeconds ?? transmitted / 32000,
      measurement: usage.audioSeconds === undefined ? 'local' : 'provider', outcome: complete ? 'succeeded' : 'uncertain' });
  }
  ...
  function done(error?: Error) {
    if (settled) return; meter(undefined, !error); settled = true; clearTimeout(timer); clearTimeout(finishTimer); opts.signal.removeEventListener('abort', abort);
  ...
  function flush() {
    ...
    try {
      for (const bytes of queue) { if (qwen3) json({ event_id: randomUUID(), type: 'input_audio_buffer.append', audio: bytes.toString('base64') }); else socket.send(bytes); transmitted += bytes.length; }
      queue = []; queuedBytes = 0;
      if (ending) {
  ...
  socket.onOpen(() => {
    ...
    try {
      ...
      else json({ header: { action: 'run-task', task_id: taskId, streaming: 'duplex' }, payload: { task_group: 'audio', task: 'asr', function: 'recognition', model: opts.model, parameters: { format: 'pcm', sample_rate: 16000, ...vocabulary }, input: {} } });
  ...
  socket.onMessage(raw => {
    ...
    try {
      if (qwen3) {
        ...
        if (message.usage) meter(message);
        ...
        if (message.sentence?.final) meter(JSON.parse(raw));
