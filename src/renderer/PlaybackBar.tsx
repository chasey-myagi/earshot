...
export function PlaybackBar({ playback, blocked, onOpen }: {
  ...
  const label = loading ? '正在载入录音' : failed ? '回听未完成' : playing ? '正在回听' : playback.status === 'ended' ? '播放结束' : '已暂停';
  const toggleLabel = loading ? '载入中' : failed ? '重试播放' : playing ? '暂停回听' : playback.status === 'ended' ? '从头播放' : '继续播放';
  return <section className="playback-bar" aria-label="回听控制" aria-busy={busy || loading}>
    <div className="playback-row">
      <button type="button" className="playback-title" title={`回到会话：${playback.title}`} onClick={() => onOpen(playback.sessionId)}>
        <strong>{playback.title}</strong><span role="status">{label}</span>
      </button>
      <div className="playback-transport">
        <button type="button" className="playback-skip" aria-label="后退 5 秒" disabled={busy || loading || failed || blocked} onClick={() => skip(-5)}>−5 秒</button>
        <button type="button" className="playback-toggle" disabled={busy || loading || blocked} aria-label={toggleLabel} title={toggleLabel}
          onClick={() => void run(() => playing ? window.earshot.pausePlayback(playback.sessionId) : window.earshot.playSession(playback.sessionId))}>
          <span className="t-icon-swap" data-state={playing ? 'b' : 'a'} aria-hidden="true"><span className="t-icon" data-icon="a"><PlayIcon /></span><span className="t-icon" data-icon="b"><PauseIcon /></span></span>
        </button>
        <button type="button" className="playback-skip" aria-label="前进 5 秒" disabled={busy || loading || failed || blocked} onClick={() => skip(5)}>+5 秒</button>
      </div>
      <div className="playback-tools">
        <select className="playback-rate" aria-label="播放速度" value={playback.rate} disabled={busy || loading || failed || blocked}
        ...
        <button type="button" className="icon-button" aria-label="结束回听" title="结束回听" disabled={busy} onClick={() => void run(window.earshot.stopPlayback)}><CloseIcon /></button>
      </div>
    </div>
    <div className="playback-seek">
      <output>{formatClock(draft ?? playback.positionSec)}</output>
      <input type="range" aria-label="播放位置" aria-valuetext={`${formatClock(draft ?? playback.positionSec)}，共 ${formatClock(playback.durationSec)}`}
        ...
        onBlur={seek} />
      <span className="playback-duration">{formatClock(playback.durationSec)}</span>
    </div>
    {error || playback.error ? <p role="alert" className="playback-message error">{error ?? playback.error}</p>
