...
export function SessionList({ sessions, selectedId, navigation, settingsOn, onSelect, onSettings, onSearchSelect, searchRefreshKey }: {
  ...
  async function remove(targets: string[]): Promise<ActionResult> {
    ...
    try {
      for (const row of rows) {
        try {
          const result = await window.earshot.deleteSession(row.id);
      ...
      const message = failed.length ? `${failed.length} 个会话未删除：${[...new Set(messages)].join('；')}` : null;
      setError(message);
  ...
  return <aside className="slist">
    ...
    <div className="session-filter" role="group" aria-label="筛选会话">
    ...
    <div ref={list} className="sl-main" tabIndex={0} aria-label="会话列表" onKeyDown={event => {
      ...
      if (event.key === 'Escape') { event.preventDefault(); update({ ids: [], anchor: null }); }
    }}>
      ...
      {groups.map(group => group.rows.length > 0 && <div className="session-group" key={group.label}>
        ...
        <div className="session-group-rows">{group.rows.map(row => {
          ...
          return <SessionRow key={row.id} row={row} selected={chosen.includes(row.id)} active={row.id === selectedId && !settingsOn}
            ...
            }} deleteTargets={targets} deletionBlocked={busy || sessions.some(item => targets.includes(item.id) && item.status === 'recording')}
            interactionDisabled={busy} onDelete={remove} />;
        })}</div>
      </div>)}
    </div>
    {(chosen.length > 1 || error || busy) && <div className="session-selection-bar">
      <div className="session-selection-actions"><span role="status">{busy ? '正在删除…' : `已选择 ${chosen.length} 项`}</span>
        <button type="button" disabled={busy || !chosen.length || protectedSelection} title={protectedSelection ? '请先停止录制并保存' : '删除所选会话（⌘⌫）'} onClick={() => void remove(chosen)}>删除</button></div>
